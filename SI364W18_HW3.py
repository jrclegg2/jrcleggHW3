...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://jack:jack@localhost/jrcleggHW3"
## Provided:
...
# this web app will be able to fill in information about tweets they wish existed
...
## -- display_name: the display name of the twitter user with that username
##                  (Required, + set up custom validation for this -- see below)

# HINT: Check out index.html where the form will be rendered to decide what field names to use in the form class definition
class addTweet(FlaskForm):
...
# - the display name MUST be at least 2 words (this is a useful technique to practice,
# - even though this is not true of everyone's actual full name!)
...
def countLong(tweets):
    ...
    for tweet in tweets:
        ...
        for char in tweet.text:
            if char != ' ':
                numCount += 1
        tweetCount[tweet] = numCount
    longest = sorted(tweetCount, key = lambda x: x.text)
    longest = sorted(longest, key = lambda x: tweetCount[x], reverse = True)[0]
    username = User.query.filter_by(id = longest.user_id).first().username
    values = (longest.text, username)
    return values
...
def index():
    ...
    if form.validate_on_submit():
        textSubmit = form.text.data
        usernameSubmit = form.username.data
        display_nameSubmit = form.display_name.data
    ...
    ## Or if there is not, then create one and add it to the database
        userCheck = User.query.filter_by(username = usernameSubmit).first()
        if not userCheck:
            userCheck = User(username = usernameSubmit, display_name = display_nameSubmit)
    ...
    ## And redirect to the list of all tweets
        tweetCheck = Tweet.query.filter_by(user_id = user.id, text = textSubmit).first()
        if tweetCheck:
            flash("Tweet already exists...")
            return redirect(url_for('see_all_tweets'))

    ## Assuming we got past that redirect,S
    ## Create a new tweet object with the text and user id
    ## And add it to the database
    ## Flash a message about a tweet being successfully added
    ## Redirect to the index page
        else:
            tweet = Tweet(user_id = user.id, text = textSubmit)
...
def see_all_tweets():
    ...
    tweets_users = [(tweet.text, User.query.filter_by(id = tweet.user_id).first().username) for tweet in tweets]
...
def get_longest_tweet():
    tweets = Tweet.query.all()
    return render_template('longest_tweet.html', longest = countLong(tweets))
# NOTE:
